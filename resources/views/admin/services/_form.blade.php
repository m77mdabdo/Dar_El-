...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div>
        <label class="dj-admin-label">{{ __('admin_services.title_en') }}</label>
    ...
    <div>
        <label class="dj-admin-label">{{ __('admin_services.title_ar') }}</label>
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
    <div>
        <label class="dj-admin-label">{{ __('admin_services.description_en') }}</label>
    ...
    <div>
        <label class="dj-admin-label">{{ __('admin_services.description_ar') }}</label>
...
<div class="mt-4 flex items-end gap-4">
    <div class="flex-1">
        <label class="dj-admin-label">{{ __('admin_services.icon') }}</label>
        <select name="icon" class="dj-admin-input">
            ...
            @foreach (\App\Models\Service::ICONS as $djIconKey => $djIconSvg)
                <option value="{{ $djIconKey }}" {{ $djSelectedIcon === $djIconKey ? 'selected' : '' }}>{{ __('admin_services.icon_'.$djIconKey) }}</option>
            @endforeach
        </select>
...
<label class="dj-admin-checkbox-row mt-4">
    ...
    {{ __('admin_services.is_active') }}
</label>

<button type="submit" class="dj-admin-btn dj-admin-btn-primary mt-4">{{ __('admin_services.save_service') }}</button>
